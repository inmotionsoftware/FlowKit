...
open class FlowViewController<Args,Return>: UIViewController, Flow {
    ...
    public var isResolved: Bool {
        guard let promise = self.promise else { return true }
        guard let p = promise.promise else { return true }
        return p.isResolved
    }
...
extension FlowViewController: BackButtonDelegate {
...
// Resolving
extension FlowViewController: Resolvable {
...
extension FlowViewController: Cancelable {
    public func cancel() {
        self._resolve(result: .cancel)
    }
}

extension FlowViewController: Backable {
    public func back() {
...
extension FlowViewController where Args == Void  {
